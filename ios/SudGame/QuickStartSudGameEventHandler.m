...
@interface QuickStartSudGameEventHandler()
@property(nonatomic,strong)SudGmSuccessStringBlock codeSuccessBlock;
@property(nonatomic,strong)SudGmFailedBlock codeFailBlock;
@end
...
- (nonnull GameViewInfoModel *)onGetGameViewInfo {
  ...
  if (self.gameRect) {
    m.view_game_rect = self.gameRect;
  }
  return m;
}
...
- (void)onGetCode:(nonnull NSString *)userId success:(nonnull SudGmSuccessStringBlock)success fail:(nonnull SudGmFailedBlock)fail {
  self.codeSuccessBlock = success;
  self.codeFailBlock = fail;
  ...
  [self.plugin handleOnGetCode:userId appId:self.loadConfigModel.appId];
...
- (void)updateCode:(NSString *)code {
  if (code.length ==0) {
    if (self.codeFailBlock) {
      self.codeFailBlock(-1, @"code is empty");
    }
    [self clearBlock];
    return;
  }
  if (self.codeSuccessBlock) {
    self.codeSuccessBlock(code);
  }
  [self clearBlock];
}

- (void)clearBlock {
  self.codeFailBlock = nil;
  self.codeSuccessBlock = nil;
}
...
- (BOOL)onGameStateChange:(id<ISudFSMStateHandle>)handle state:(NSString *)state dataJson:(NSString *)dataJson {
  [self.plugin onGameStateChanged:state dataJson:dataJson];
  return true;
}

- (BOOL)onPlayerStateChange:(id<ISudFSMStateHandle>)handle userId:(NSString *)userId state:(NSString *)state dataJson:(NSString *)dataJson {
  [self.plugin onPlayerStateChanged:state dataJson:dataJson];
  return true;
}
...
- (void)notifyStateChange:(NSString *)state dataJson:(NSString *)dataJson {
  [self.sudFSTAPPDecorator notifyStateChange:state dataJson:dataJson];
}
@end
